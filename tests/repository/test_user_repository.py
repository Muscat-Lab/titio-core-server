...
from tests.repository.fixture import new_user, performer_repository, user_repository
...
class TestUserRepository:
    ...
    async def test_get_user_by_email(
        ...
        assert await user_repository.get_user_by_email(email=user.email) == user
    ...
    async def test_get_user_by_kakao_id(
        ...
        assert (
            await user_repository.get_user_by_kakao_id(kakao_id=user.kakao_id) == user
        )
    ...
    async def test_get_user_by_id(
        ...
        assert await user_repository.find_user_by_id(user_id=user.id) == user
    ...
    async def test_find_user_by_username(
        ...
        assert (
            await user_repository.find_user_by_username(username=user.username) == user
        )
    ...
    async def test_find_user_by_email(
        ...
        assert await user_repository.find_user_by_email(email=user.email) == user
